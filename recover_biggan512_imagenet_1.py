...
parser.add_argument('--img_dir', default='./ILSVRC2012_val_00005150.JPEG', \
                    type=str, metavar='ID', help='target image')
parser.add_argument('--class_id', default='n01833805', type=str, help='imagenet class id')
...
basic_path = args.train_url + args.class_id
...
def F_inverse(model, netG, input, class_vector, features_ini, feature_objective):
    ...
    noise_vector = torch.tensor(
        truncated_noise_sample(truncation=truncation, batch_size=1, seed=int(args.noise_seed))
    ...
    save_as_images(fake_img.detach().cpu().numpy(), '{0}/step1_init_fake'.format(basic_path))
    ...
    for epoch in range(args.epoch1):
        ...
        if epoch % 500 == 0:
            ...
            save_as_images(fake_img.detach().cpu(), '{0}/step1_epoch_{1}'.format(basic_path, epoch//500))

        features_ini = features_ini.cuda()
        input = input.cuda()    # input已经预处理
        input_interpolate = F.interpolate(input, size=(512, 512), mode='bilinear', align_corners=True).cuda()
        ...
        fake_img_PIL = convert_to_images(fake_img.cpu())[0] # print('fake_img_PIL:', fake_img_PIL)
        fake_img_norm = trans(fake_img_PIL).unsqueeze(0).cuda()
        fake_img_norm_interpolate = F.interpolate(fake_img_norm, size=(512, 512), mode='bilinear', align_corners=True).cuda()
...
def train(input, target, netG, model):
    ...
    save_as_images(_image_restore(_image_norm(_image_restore(input_interpolate))).cpu().detach(),
    ...
    F_inverse(model, netG, input, onehot_class_vector, feature_ini, feature_objective)
