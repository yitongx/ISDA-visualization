'''
源自generate_biggan_imagenet_1
step1: 进行图像对应噪声的recovery
随机噪声，自始至终用正则化噪声训练
找到了梯度消失的问题在于PIL转换
加入了vgg16中间层作为损失, mode='sum'
可以自由选定loss1成分
'''
...
parser.add_argument('--schedule', nargs='+', default=None, type=int, metavar='SC', help='step1 schedule')
...
parser.add_argument('--truncation', default=0.4, type=float, metavar='TRUNC',
...
parser.add_argument('--aug_number', default=32, type=int, metavar='AN',
                    help='number of augment results for one real image')
parser.add_argument('--aug_alpha', default=0.2, type=float, metavar='AA', help='For feature augmentation control')
parser.add_argument('--eta', default=0.01, type=float, metavar='ETA', help='For step1 loss ratio')
...
# mean_mask = torch.Tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1).expand(1, 3, args.size, args.size).cuda()
# std_mask = torch.Tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1).expand(1, 3, args.size, args.size).cuda()
...
# basic_path = args.train_url + args.class_id + '_' + args.job_id + '_' + args.name
basic_path = args.train_url \
            + str(args.job_id) + '_' + str(date.month) + '_' + str(date.day) + '_' + str(localtime.tm_hour) + '_' + str(localtime.tm_min)\
            +'_' + str(class_id) + '_' + str(img_id)\
            +'_epoch1_' + str(args.epoch1) \
            +'_lr_' + str(args.lr1) \
            +'_eta_' + str(args.eta) \
            +'_trunc_' + str(args.truncation)
if args.name:
    basic_path += '_' + str(args.name) + '_' + str(args.loss_component)
...
def F_inverse(model, netG, vgg, input, class_vector, features_ini, feature_objective):
    truncation = args.truncation
    noise_vector = torch.nn.Parameter(torch.randn(1, 128, requires_grad=True).cuda())
    # noise_vector = noise_vector.cuda()
    '''
    noise_vector = torch.nn.Parameter(torch.tensor(
        truncated_noise_sample(truncation=truncation, batch_size=1, seed=args.noise_seed)
